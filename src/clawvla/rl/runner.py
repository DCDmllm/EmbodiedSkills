...
from .config import RLConfig, dump_resolved_config, load_rl_config
...
from .trajectory import EpisodeRecord, TrajectoryWriter, build_response_mask_from_calls, load_jsonl
...
def _preflight(config: RLConfig, *, mode: str, static_policy: bool = False) -> dict[str, Any]:
    ...
    try:
        ...
        registry.handler_for_task(config.rollout.task_name)
    ...
    if mode == "train":
        try:
            subprocess.run(
                [config.trainer.python, "-c", "import verl, torch, vllm; import clawvla.rl.verl_agent_loop"],
                check=True,
                stdout=subprocess.PIPE,
                stderr=subprocess.STDOUT,
                text=True,
                env=command_env(config.trainer, {"PYTHONPATH": "/mnt/wangwai/vla/clawvla/src"}),
                timeout=60,
            )
        except Exception as exc:
...
def _run_rollout_only(
    ...
    *,
    ...
    try:
        for episode_index in range(config.rollout.episodes):
            seed = config.rollout.seeds[episode_index % len(config.rollout.seeds)]
            before_call_count = len(proxy.calls)
            terminal.event("episode_start", index=episode_index, seed=seed, task=config.rollout.task_name)
...
def _write_verl_dataset(config: RLConfig, run_dir: Path, *, split: str) -> Path:
    ...
    count = max(1, config.rollout.episodes if split == "train" else min(2, config.rollout.episodes))
    with path.open("w", encoding="utf-8") as handle:
        for index in range(count):
            seed = config.rollout.seeds[index % len(config.rollout.seeds)]
            row = {
                "prompt": [
                    {
                        "role": "user",
                        "content": (
                            ...
                            f"the task: {config.rollout.instruction}"
                        ),
                    }
                ],
                "data_source": "clawvla_robotwin",
                "agent_name": "clawvla_agent",
                "reward_model": {"ground_truth": config.rollout.task_name},
                "extra_info": {
                    "index": index,
                    "seed": seed,
...
def _replay_adapter(path: str | None, terminal: RLTerminal) -> None:
    ...
    for episode in episodes:
        ...
        if traces:
            build_response_mask_from_calls(traces)
            checked += 1
    terminal.event("adapter_replay", episodes=len(episodes), checked=checked)
