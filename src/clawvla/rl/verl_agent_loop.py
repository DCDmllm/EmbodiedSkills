...
from .trajectory import TrajectoryWriter, build_agent_loop_adapter_from_calls
...
class VerlServerManagerBackend:
    ...
    async def _generate_async(self, request: dict[str, Any]) -> PolicyGeneration:
        ...
        log_probs = list(output.log_probs[:response_length]) if getattr(output, "log_probs", None) else []
...
class ClawVLAAgentLoop(AgentLoopBase):  # type: ignore[misc,valid-type]
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs: Any) -> Any:
        ...
        adapter = build_agent_loop_adapter_from_calls(
            episode.policy_calls,
            separator_ids=self.tokenizer.encode("\n", add_special_tokens=False),
            require_multimodal_payload=True,
        )
